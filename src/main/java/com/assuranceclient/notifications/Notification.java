package com.assuranceclient.notifications;
...
public class Notification extends javax.swing.JComponent {
    ...
    public Notification(Frame fram, Type type, Location screenLocation, String message) {
        this.fram = fram;
        this.type = type;
        ...
        TimingTarget target = new TimingTargetAdapter() {
            ...
            public void begin() {
                if (!showing) {
                    ...
                    int margin = 10;
                    int y = 0;
                    if (screenLocation == Location.TOP_CENTER) {
                        x = fram.getX() + ((fram.getWidth() - dialog.getWidth()) / 2);
                        y = fram.getY();
                        top_to_bot = true;
                    } else if (screenLocation == Location.TOP_RIGHT) {
                        x = fram.getX() + fram.getWidth() - dialog.getWidth() - margin;
                        y = fram.getY();
                        top_to_bot = true;
                    } else if (screenLocation == Location.TOP_LEFT) {
                        x = fram.getX() + margin;
                        y = fram.getY();
                        top_to_bot = true;
                    } else if (screenLocation == Location.BOTTOM_CENTER) {
                        x = fram.getX() + ((fram.getWidth() - dialog.getWidth()) / 2);
                        y = fram.getY() + fram.getHeight() - dialog.getHeight();
                        top_to_bot = false;
                    } else if (screenLocation == Location.BOTTOM_RIGHT) {
                        x = fram.getX() + fram.getWidth() - dialog.getWidth() - margin;
                        y = fram.getY() + fram.getHeight() - dialog.getHeight();
                        top_to_bot = false;
                    } else if (screenLocation == Location.BOTTOM_LEFT) {
                        x = fram.getX() + margin;
                        y = fram.getY() + fram.getHeight() - dialog.getHeight();
                        top_to_bot = false;
                    } else {
    ...
    private void initComponents() {
        ...
        cmdClose.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/close.png")));
